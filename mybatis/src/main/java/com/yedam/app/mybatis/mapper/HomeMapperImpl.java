...
public class HomeMapperImpl implements EmpMapper {
	...
	public EmpVO selectInfo(EmpVO emp) {
		try (SqlSession session = factory.openSession()) {
			EmpVO find = session.selectOne("com.yedam.app.mybatis.mapper.EmpMapper.selectInfo", emp.getEmployeeId());
			return find;
		}
	}
	...
	public int updateInfo(EmpVO emp) {
		try (SqlSession session = factory.openSession(true)) {
			int result = session.insert("com.yedam.app.mybatis.mapper.EmpMapper.updateInfo", emp);
			return result;
		}
	}
